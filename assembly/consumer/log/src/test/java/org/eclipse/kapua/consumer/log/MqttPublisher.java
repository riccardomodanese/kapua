...
public class MqttPublisher {
    ...
    public static void main(String[] argv) throws MqttSecurityException, MqttException {
        ...
        String topic = "textLogMessages";
        for (int i=0; i<10; i++) {
            logger.info("Publishing RFC 5424 messages...");
            for (int j=0; j<MESSAGES_5424.length; j++) {
                logger.info("Publishing message: {}", MESSAGES_5424[j]);
                client.publish(topic, getLogMessage(MESSAGES_5424[j], clientId));
            }
            logger.info("Publishing RFC 3164 messages...");
            for (int j=0; j<MESSAGES_3164.length; j++) {
                logger.info("Publishing message: {}", MESSAGES_3164[j]);
                client.publish(topic, getLogMessage(MESSAGES_3164[j], clientId));
            }
        }
        client.disconnect();
    }
